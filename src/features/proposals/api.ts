...
const buildCustomerName = (deal: DealRecord | undefined): string => {
  ...
  const contactName =
    `${deal.contact?.first_name ?? deal.first_name ?? ""} ${deal.contact?.last_name ?? deal.last_name ?? ""}`.trim();
...
): Promise<{ rows: ProposalListRow[]; summary: ProposalListSummary }> => {
  const [quotes, deals] = await Promise.all([
    apiClient<QuoteRecord[]>("/quotes", { params: { company_id: companyId } }).catch(
      (error) => {
        console.error("Failed to load quotes", error);
        return [] as QuoteRecord[];
      }
    ),
    apiClient<DealRecord[]>("/deals", { params: { company_id: companyId } }).catch(
      (error) => {
