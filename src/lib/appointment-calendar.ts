import type { AppointmentCalendarRecord, AppointmentCalendarEvent } from "@/types/appointments";
...
export const mapAppointmentsToCalendarEvents = (
  ...
  return appointments
    .map((appointment) => {
      const deal = Array.isArray(appointment.deal) ? appointment.deal[0] ?? null : appointment.deal ?? null;

      const dealId = deal?.id ?? appointment.deal_id ?? null;
      if (!dealId) return null;

      const contact = deal && Array.isArray(deal.contact) ? deal.contact[0] ?? null : deal?.contact ?? null;

      const contactNameFromContact = `${contact?.first_name ?? ""} ${contact?.last_name ?? ""}`.trim();
      const contactNameFromDeal = `${deal?.first_name ?? ""} ${deal?.last_name ?? ""}`.trim();
      ...
      const contactNameFromMap = contactId ? contactNameByContactId?.get(contactId) ?? "" : "";
