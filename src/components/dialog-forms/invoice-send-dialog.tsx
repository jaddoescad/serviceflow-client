...
import { INVOICE_DELIVERY_METHOD_LABELS, INVOICE_DELIVERY_METHOD_ORDER } from "@/constants/invoice-delivery";
...
export type InvoiceSendDialogProps = {
  ...
  sendMethod: InvoiceDeliveryMethod;
  onSendMethodChange: (method: InvoiceDeliveryMethod) => void;
...
export function InvoiceSendDialog({
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      ariaLabel={`Send Invoice #${invoiceNumber}`}
      size="lg"
      align="center"
    >
      <ModalHeader
        title={`Send Invoice #${invoiceNumber}`}
        subtitle="Share the invoice link via email, text, or both."
        onClose={onClose}
      />

      {errorMessage ? (
        <div className="mx-5 mt-3 rounded-md border border-rose-200 bg-rose-50 px-3 py-2 text-[12px] text-rose-600">
          {errorMessage}
        </div>
      ) : null}
      ...
      <ModalBody className="space-y-4">
        <section className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
          ...
          <div className="mt-2 grid grid-cols-3 gap-2">
            {INVOICE_DELIVERY_METHOD_ORDER.map((option) => {
              ...
              return (
                ...
                >
                  {INVOICE_DELIVERY_METHOD_LABELS[option]}
                </button>
              );
            })}
          </div>
          ...
          <section className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
            <div className="space-y-3">
              ...
              <Textarea
                ...
                rows={4}
                hint="We replace {{invoice_button}} with the live invoice link."
                className="bg-white"
              />
            </div>
          </section>
