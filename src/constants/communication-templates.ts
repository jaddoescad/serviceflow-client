...
export const COMMUNICATION_TEMPLATE_DEFINITIONS: Record<CommunicationTemplateKey, CommunicationTemplateDefinition> = {
  ...
  invoice_payment_request: {
    ...
    defaultEmailSubject: "Payment request for invoice {invoice-number} – {company-name}",
    defaultEmailBody:
  ...
  work_order_dispatch: {
    ...
    defaultSmsBody:
      "Work order from {company-name}: {work_order_button}",
    keywords: [
      "{company-name}",
      "{company-phone}",
      "{company-website}",
  ...
  job_schedule: {
    ...
    defaultSmsBody: "Your job is scheduled for {job-date} at {job-time}. {job-address}",
    keywords: [
      "{company-name}",
      "{company-phone}",
      "{company-website}",
